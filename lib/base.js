...
class Base {
  ...
  getFileName(filePath, ext = undefined) {
  ...
  prePlugin(name) {
    if (name.startsWith('plugin-private://')) {
      let n = name.replace('plugin-private://', '__plugin__/')
      console.log('plugin :', name)
      // console.log('plugin new :', n)
      return n
  ...
  preFilename(fileList) {
    fileList.filter(i => i.name.includes('plugin')).forEach(item => {
